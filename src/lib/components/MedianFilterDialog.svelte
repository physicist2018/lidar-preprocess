<script>
	import { files, medianFilter, medianFiltering, MEDIAN_WINDOW_MAX } from '$lib/state/store';
	...
	function isApplyDisabled() {
		return totalFiles === 0 || parseWindowSize() == null;
	}

	async function handleApply() {
		const size = parseWindowSize();
		if (size == null) return;
		await medianFiltering(size);
		onClose?.();
	}
</script>
...
<div class="fixed inset-0 flex items-center justify-center bg-black/40 p-4" style="z-index: 10000">
	...
	>
		<div class="flex items-center justify-between border-b border-gray-200 px-4 py-3">
			<span class="text-sm font-semibold text-gray-700">Медианная фильтрация</span>
			<button
				onclick={() => onClose?.()}
				aria-label="Закрыть"
		...
		<div class="min-h-0 flex-1 space-y-4 overflow-auto px-4 py-4">
			...
			{:else}
				<div>
					...
					/>
					{#if windowSize !== '' && windowSize != null && parseWindowSize() == null}
